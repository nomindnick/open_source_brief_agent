...
- **In-brief user feedback loop.** Append a pre-filled `## Feedback` heading at the bottom of each brief. The user types into it within Obsidian during/after reading. The next morning's run begins with a *feedback integration* stage: a non-agentic LLM call that reads yesterday's brief + feedback + current `Interests.md`, and produces an updated `Interests.md` (or sibling file). Same architectural pattern as Sprint 4.2's reflection — one LLM call, structured inputs, file output. Deferred from MVP for two reasons: (1) you have to live with the brief for a week before you know what feedback you actually want to give, and (2) `Interests.md` is already a hand-edit escape hatch, so the gap this fills is convenience, not capability. Pencilled in as a future sprint after a couple of weeks of MVP runs reveal the actual feedback shape.

- **Periodic memory compaction.** As `Reflections/` (and eventually user feedback notes) accumulate over weeks, the filter's `{{recent_reflections}}` context grows. A monthly (or every-N-runs) consolidation pass — non-agentic LLM call — distills the last N days of reflections + feedback into 2–5 stable themes appended to `Interests.md`, then archives the source files into `memory/Reflections/_archive/`. Keeps the active memory window small while preserving long-term signal. MVP avoids this concern by injecting only the most recent reflection into the filter (~500 tokens), but the file layout (one reflection per day, named for archivability) is chosen so this consolidation drops in cleanly later. Same pattern again — one LLM call, file in/file out, no agent loop.
