...
**Objective:** A `ModelInterface` abstraction with working llama.cpp and Ollama implementations, selected by config.

**Tasks:**
- [ ] `src/model/base.py`: define `ModelInterface` (ABC or Protocol) with a single `complete(messages: list[dict], **kwargs) -> str` method. Keep it sync for v1.
- [ ] `src/model/llamacpp.py`: implementation hitting `llama-server`'s OpenAI-compatible `/v1/chat/completions` endpoint via httpx
- [ ] `src/model/ollama.py`: implementation hitting Ollama's `/api/chat` endpoint via httpx
- [ ] `src/config.py`: pydantic-settings model loading `config.toml` + `.env`; includes `model_backend: Literal["llamacpp", "ollama"]`, `model_name`, `model_base_url`, `temperature`, `max_tokens`, `vault_path`, `iteration_cap`
- [ ] Factory function `get_model(config) -> ModelInterface`
- [ ] Manual smoke test script `scripts/smoke_model.py` that prints a model's response to "Say hello."

**Acceptance Criteria:**
- With llama.cpp running locally, `python scripts/smoke_model.py` returns a non-empty string
- Switching `model_backend = "ollama"` in `config.toml` and re-running uses Ollama with no code changes
- Bad config (missing field, bad URL) raises a clear validation error, not a stack trace deep in httpx
...
- Tool call → tool result → next model turn is visible in stdout (basic logging is enough for now; rich traces come next sprint)
...
**Tasks:**
- [ ] `src/trace/writer.py`: `TraceWriter` that opens `traces/YYYY-MM-DD/<mission>/trace.jsonl` and `trace.md` on init. Methods: `log_thought`, `log_tool_call`, `log_tool_result`, `log_parse_error`, `log_final_answer`, `close`.
- [ ] Each JSONL line is a dict with `timestamp`, `event_type`, and event-specific fields.
- [ ] Markdown version renders as a chronological narrative with section headers per turn — readable on phone via Obsidian.
- [ ] Wire `TraceWriter` into the agent loop. Pass it into `run_agent`.
...
**Tasks:**
- [ ] Verify `hf` CLI is installed and authenticated on the dev machine. Add an install note to README.
- [ ] `src/tools/hf_papers.py`:
  ...
  - `HfPapersReadTool`: input `{"id": "<arxiv-id>"}`; subprocess-calls `hf papers read <id>`; returns markdown content.
